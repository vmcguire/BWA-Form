# BWA-Form — Read this before doing anything
...
can clone, browse, and reason about BWA-Form on its own without
seeing the rest of the BWA monorepo.

All real work on BWA-Form happens inside the BWA monorepo at:

```
~/Developer/bwa-fx-ecosystem/plugins/form/
    dsp/           ← authoritative DSP implementation (THE source of truth)
    ui/            ← UI surface (Metal-rendered)
    plugin/        ← VST3 wrapper → builds BWA-Form.vst3
    manifest.json  ← name, controls, license tier, ecosystem benefits
```

If you've been asked to "work on BWA-Form," **navigate to the
...
1. **Standalone VST3** (this repo's release artifact) — `BWA-Form.vst3`
...
- **Role:** Emotion-shaped song scaffold generator + Roll/Fill/Transition auto-placement.
- **Pain attacked:** Blank timeline is the failure mode of every DAW. Hand the user structure, not a void.
- **Scope verdict:** project-global (it shapes the timeline)
- **Band tier behavior:** Not band-tier-gated; tier-gated by genre library depth.
...
4. **[`BWA-Architecture/docs/MARKET-PAIN-AND-ATTACKS.md`](https://github.com/vmcguire/BWA-Architecture/blob/main/docs/MARKET-PAIN-AND-ATTACKS.md)**
   — the row for BWA-Form. Internalize the pain + the attack before
...
- ❌ Committing to this repo. Go to the monorepo (`plugins/form/`).
...
- ❌ Adding new shared/* modules without checking if the capability folds
  ...
  Methodology Rule 3."
- ❌ Skipping the BWA-Form row in MARKET-PAIN-AND-ATTACKS.md. If the
  proposed work doesn't advance the BWA Attack for this plugin, push back.

## What "done" looks like

A `BWA-Form.vst3` that:
...
```bash
git subtree push --prefix=plugins/form \
  git@github.com:vmcguire/BWA-Form.git main
```

Then on GitHub:

1. Create a Release from `main` in this repo.
2. Attach the notarized `BWA-Form.vst3` zip.
3. Reference the monorepo commit SHA in the release notes.

---

_End of LLM onboarding for BWA-Form._
